...
function BentoGrid({ recipes, loading }: { recipes: Recipe[]; loading: boolean }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
      {/* Hero card */}
      <section className="md:col-span-8 group relative overflow-hidden rounded-[2rem] bg-surface-container-low min-h-[400px] flex flex-col justify-end p-8 transition-all hover:scale-[1.01] duration-500 shadow-2xl border border-outline/10">
        {featured ? (
          <>
            <div className="absolute inset-0 z-0">
              {featured.coverImageUrl && (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={featured.coverImageUrl} alt={featured.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 opacity-90" />
              )}
              <div className="absolute inset-0 bg-gradient-to-t from-on-surface/90 via-on-surface/40 to-transparent" />
            </div>
            <div className="relative z-10 flex flex-col gap-4">
              <div className="flex gap-2">
                {featured.isFamilyRecipe && (
                ...
                {featured.region && (
                  <span className="px-3 py-1 bg-surface/20 backdrop-blur-md text-white font-label text-[10px] font-bold tracking-widest uppercase rounded-full border border-outline/20">{featured.region.name}</span>
                )}
              </div>
              <h3 className="font-headline text-4xl font-bold text-white tracking-tight max-w-md">{featured.title}</h3>
              {featured.dietaryTags && featured.dietaryTags.length > 0 && (
                <p className="text-white/80 font-body text-lg max-w-lg capitalize">{featured.dietaryTags.join(' · ')}</p>
              )}
              <div className="pt-2">
                <Link href={`/recipes/${featured.id}`} className="inline-flex items-center gap-2 bg-gradient-to-r from-primary to-tertiary text-on-primary px-8 py-3 rounded-full font-label font-bold text-sm uppercase tracking-widest hover:shadow-xl transition-all active:scale-95">
          ...
          <div className="relative z-10 flex flex-col gap-4">
            <div className="flex gap-2">
              <span className="px-3 py-1 bg-tertiary text-on-tertiary font-label text-[10px] font-bold tracking-widest uppercase rounded-full">Global Recipes</span>
            </div>
