            ...
            <div className="space-y-4">
              ...
              {[
                ...
                <div key={key}>
                  ...
                  <div className="flex gap-2">
                    {[1, 2, 3, 4, 5].map(v => (
                      <button key={v} onClick={() => setForm(f => ({ ...f, [key]: v }))} className={`flex-1 py-2 rounded-xl text-lg transition-all ${(form as Record<string, number>)[key] === v ? 'bg-primary/10 scale-110' : 'bg-surface-container hover:bg-surface-container-high'}`}>
                        {labels[v - 1]}
                      </button>
                    ))}
                  </div>
